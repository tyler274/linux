...
//! First-fit VA search for `vm_unmapped_area`, plus `find_vma_prev`.
//!
//! Bottom-up and top-down walks of existing VMAs replace the maple-tree
//! walk in `unmapped_area()` / `unmapped_area_topdown()`. `find_vma_prev`
//! is a hit vs next-VMA sequencer; the iterator walk stays in C.
...
/// Matches `RUST_FVP_*` in `include/linux/mm.h`.
const FVP_HIT: i32 = 0;
/// Iterator load missed; advance to the next VMA.
const FVP_NEXT: i32 = 1;

static N_FVP: Atomic<u32> = Atomic::new(0);

/// C ABI: sequence `find_vma_prev` after C loads the iterator.
///
/// Sets `*handled` once classify has run. `vma_next` stays in C.
///
/// # Safety
///
/// `s` must be a live `rust_fvp_state` with `mm`/`addr`/`pprev` filled.
/// mmap lock held. `handled` must be a valid out-parameter.
#[no_mangle]
pub unsafe extern "C" fn rust_fvp_dispatch(
    s: *mut bindings::rust_fvp_state,
    handled: *mut c_int,
) -> *mut bindings::vm_area_struct {
    ...
    // SAFETY: `s` is live; classify loads the iterator and may set `*pprev`.
    let kind = unsafe { bindings::rust_fvp_classify(s) };
    // SAFETY: `handled` is valid.
    unsafe { *handled = 1 };

    let prev = N_FVP.fetch_add(1, Relaxed);
    if prev == 0 {
        pr_info!("rust-mmap: first find_vma_prev sequenced\n");
    }

    match kind {
        // SAFETY: Iterator hit this address.
        FVP_HIT => unsafe { bindings::rust_fvp_hit(s) },
        // SAFETY: Miss; return the next VMA if any.
        FVP_NEXT => unsafe { bindings::rust_fvp_next(s) },
        _ => {
            pr_err!("rust-mmap: unknown find_vma_prev kind {kind}\n");
            unsafe { bindings::rust_fvp_abort(s) };
            ptr::null_mut()
        }
    }
}
